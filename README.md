...
Update grocery prices in `data.js` each Friday:

- `townName`
- `lastUpdated`
- `nextUpdate`
- item prices in `items`

Fuel prices change faster than groceries and can be updated more often.

Use `null` when a store does not carry a comparable item. Displayed prices are intended to be shelf prices with no coupons applied.

## Deploy

Cloudflare Pages can deploy this repository as a static site:

- Build command: none
- Build output directory: `/`
- Custom domain: `jacksonprices.com`

The root `CNAME` file is included so GitHub Pages-compatible hosting also recognizes the custom domain.
